...
5. COMMIT KODU (opcjonalny, ZAWSZE po potwierdzeniu): sprawdź `git status`. Jeśli są
   ...
   - Zaproponuj treść commitu wg konwencji projektu (conventional commits:
     feat:/fix:/refactor:/docs:/chore:), zgrupowaną logicznie.
   - Wykonaj commit DOPIERO po akceptacji użytkownika. Możesz zaproponować podział
     na kilka commitów, jeśli zmiany są niejednorodne.
...
9. COMMIT PLIKÓW STANU (opcjonalny, po potwierdzeniu): zaproponuj osobny commit
   obejmujący WYŁĄCZNIE pliki stanu sesji (last_session.md, MEMORY.md, ewentualnie
   last_session.archive.md / MEMORY.archive.md), oddzielony od commitu kodu:
   `chore(session): zapis stanu sesji [YYYY-MM-DD]`. Wykonaj po akceptacji. Bez push.
   Pomiń, jeśli pliki stanu nie są wersjonowane lub user nie chce ich commitować.
